...
import {
  ...
  MastheadMain, MastheadToggle, Nav, NavItem, NavList, NavExpandable,
...
// import HciLogo from '/root/reactjs/src/bgimages/Hci-Logo.svg';
...
const AppLayout = () => {
  ...
  const [isProOpen, setIsProOpen] = React.useState(false);
  const [proSelected, setProSelected] = React.useState([]);

  const onProToggle = isOpen => setIsProOpen(isOpen);

  const onProSelect = (event, selection, isPlaceholder) => {
    setProSelected(selection);
    setIsProOpen(false);
  };
  ...
  if (typeof projectList != "undefined") {
    ...
    for (let index in projectList) {
      ...
      projectOption[index] = <SelectOption
        key={index}
        value={item.project}
      />
  ...
  //const { isPopOpen, selected, isDisabled, direction, isToggleIcon } = projectSel;
  ...
  const masthead = <Masthead>
    ...
    md: '180px',
    '2xl': '220px'
  }} src={require('@app/bgimages/Patternfly-Logo.svg')} alt="LOGO">
  ...
  const pageNav = <Nav onSelect={onNavSelect}>
    <NavList>
      <NavItem itemId={0} isActive={activeItem === 0} to="#/Dashboard2">
        Dashboard
      </NavItem>
      <NavItem itemId={1} isActive={activeItem === 1} to="#/events">
        Events
      </NavItem>
