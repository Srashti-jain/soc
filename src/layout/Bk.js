...
export const AppLayout = () => {
  ...
  const onProSelect = (event, selection, isPlaceholder) => {

    alert(isPlaceholder)
    if (isPlaceholder) clearProSelection
    else {
      setProjectSel({
        selected: selection,
        isProOpen: false
 ...
 // const favPro = useSelector((state) => state.projectSel)

  const onProFavorite = (itemId, isFavorite) => {

    alert(itemId+' '+isFavorite)
    if (isFavorite) {
      let fitFavPro = projectSel.favorites.filter(id => id !== itemId)

      // console.log('fitFavPro')
      ...
      // });
        ...
        setProjectSel({
        favorites: [...projectSel.favorites, itemId]
      })
...
if(typeof projectList != "undefined")
{
  ...
  for(let index in projectList)
  {
      ...
      // <DropdownItem className={icon} key={'pro_'+index} onClick={() =>  updateProject(item.pro_id)} >
      // {item.project}
      // </DropdownItem> 

            projectOption[index] =
            <SelectOption id={item.pro_id} key={'pro_'+index} value={item.project} />
...
var proState = projectSel;

const { isProOpen, selected, favorites } = proState;
    const titleId = 'group1';

const projectDropDown =   <Select
variant={SelectVariant.typeahead}
typeAheadAriaLabel="Select value"
onToggle={onProToggle}
onSelect={onProSelect}
selections={selected}
isOpen={isProOpen}
placeholderText="Project"
aria-labelledby={titleId}
isGrouped
onFavorite={onProFavorite}
favorites={favorites}
>
...
// const projectDropDown = <Dropdown onSelect={onSelect} className="projectDropDown" toggle={<DropdownToggle toggleVariant="default" id="toggle-descriptions" onToggle={onToggle}>
// Project
