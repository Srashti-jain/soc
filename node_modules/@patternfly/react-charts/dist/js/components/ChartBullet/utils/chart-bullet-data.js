...
// destructure last
theme = ChartUtils_1.getBulletComparativeMeasureTheme(themeColor), y }) => {
    const datum = [];
...
// destructure last
theme = ChartUtils_1.getBulletComparativeErrorMeasureTheme(themeColor), y }) => exports.getComparativeMeasureData({
    data,
    theme,
    themeColor,
    y
...
// destructure last
theme = ChartUtils_1.getBulletComparativeWarningMeasureTheme(themeColor), y }) => exports.getComparativeMeasureData({
    data,
    theme,
    themeColor,
    y
...
// destructure last
theme = ChartUtils_1.getBulletPrimaryDotMeasureTheme(themeColor), y, y0 }) => exports.getComparativeMeasureData({
    data,
    invert,
    theme,
    themeColor,
...
// destructure last
theme = ChartUtils_1.getBulletPrimarySegmentedMeasureTheme(themeColor), negativeMeasureTheme = ChartUtils_1.getBulletPrimaryNegativeMeasureTheme(themeColor), y, y0 }) => {
    const negativeDatum = [];
    const positiveDatum = [];
...
// destructure last
theme = ChartUtils_1.getBulletQualitativeRangeTheme(themeColor), y, y0 }) => {
    const datum = [];
    victory_core_1.Data.formatData(data, { y, y0 }, ['y', 'y0']).forEach((dataPoint, index) => {
        datum.push(Object.assign(Object.assign({}, dataPoint), { _index: index // Save to sync legend color
         }));
